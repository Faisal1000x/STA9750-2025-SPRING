...
library(DT)
datatable(head(county_shapes, 20))
```
...
datatable(head(election_2024, 5))
```
...
datatable(head(election_2020, 5))
```
...
datatable(head(combined_elections, 5))
...
# Make sure your county_shapes is projected properly
county_shapes <- st_transform(county_shapes, crs = 4326)

# Prepare combined_elections
combined_elections <- combined_elections |>
  mutate(
...
# Create state abbreviation and full name mapping
state_df <- data.frame(STATEFP = c(sprintf("%02d", 1:50), "11"),  # include "11" for DC
                       STATE_ABB = c(state.abb, "DC"),  # append "DC" for District of Columbia
                       STATE_NAME = c(state.name, "DISTRICT OF COLUMBIA"),  # append DC name
                       stringsAsFactors = FALSE)

# Ensure there are no duplicates in state_df (should not be necessary, but good to check)
state_df <- state_df %>%
  distinct(STATEFP, .keep_all = TRUE)

# Ensure there are no duplicates in county_shapes
county_shapes <- county_shapes %>%
  distinct(STATEFP, NAME, .keep_all = TRUE)  # Assuming NAME is a unique county identifier

# Perform the left join and ensure no column name conflicts
map_data <- county_shapes |>
  mutate(STATEFP = as.character(STATEFP)) |>
  left_join(state_df, by = "STATEFP", suffix = c("", "_state"))

# Now mutate the columns based on the correct names
map_data <- map_data |>
  mutate(
    COUNTY_NAME = toupper(NAME),  # Assuming 'NAME' is the county name column
    STATE_NAME = toupper(ifelse(!is.na(STATE_NAME_state), STATE_NAME_state, "")),  # Ensure STATE_NAME exists and is not NA
    FIPS = paste0(STATE_NAME, "_", COUNTY_NAME)
  ) |>
  left_join(combined_elections, by = "FIPS")

# Check how many counties have valid shift data
...
leaflet(map_data) |>
  ...
  addPolygons(
    fillColor = ~pal(Trump_shift_pct * 100),
    color = "#666666",
    weight = 0.5,
    opacity = 1,
    fillOpacity = 0.7,
    ...
    popup = ~paste0(
      "<strong>", NAME, ", ", STATE_NAME_state, "</strong><br>",
